...
### サンドボックスとは
AIが触れる範囲をOS/コンテナレベルで物理的に制限する仕組みです。
フックやルールは「やらないでね」というお願いですが、サンドボックスは「やれない」状態を作ります。

### 有効化方法

Claude Code のサンドボックスは以下の2つの方法で有効化できます:
...
### サンドボックスの仕組み
- **macOS**: Seatbelt フレームワークを使用
- **Linux/WSL2**: bubblewrap を使用（`sudo apt install bubblewrap` で事前インストール）
- **WSL1**: サポートされていない

サンドボックスを有効にすると:
- ファイルアクセスがプロジェクトディレクトリ内に制限される
- ネットワークアクセスが許可されたドメインのみに制限される
- システムコマンドの実行が制限される

フック・ルール・パーミッションとの併用を推奨します。
...
| | サンドボックス | フック/ルール |
|---|---|---|
| 制限レベル | OS・カーネル（物理的） | アプリケーション（論理的） |
| バイパス | 不可能 | 工夫すれば可能 |
| ネットワーク制限 | できる | できない |
| うっかりミス防止 | やりすぎ（正当な操作も止める） | 適切（パターンで判断） |
...
【第0層: サンドボックス（OS レベル）】
- セッション内で /sandbox コマンド、または settings.json に "sandbox": {"enabled": true} で有効化
- macOS: Seatbelt、Linux: bubblewrap を使用
- Docker内実行が最も確実な隔離方法
- フック/ルールとの併用を推奨
...
| 層 | 守れるもの | 守れないもの |
|---|---|---|
| サンドボックス | 未知の攻撃、ネットワーク外部送信、プロジェクト外アクセス | 正当な外部通信も止める（開発体験が落ちる） |
| フック | 既知のAPIキーパターン、既知の危険コマンド | 未知のパターン、分割書き込み、エンコードされたキー |
| パーミッション | 機密ファイルの書き込み、破壊コマンド | ファイルの読み取り（Readは制限なし） |
| ルール | AIの判断が必要なもの（設計、外部通信の妥当性） | Claudeが無視する可能性はゼロではない |

→ **どの層も単体では完璧ではない。4層を重ねることで現実的な堅牢性を確保する。**
